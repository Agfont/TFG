...
def main():
    ...
    sns.scatterplot(x="pca-one", y="pca-two", hue=df.y.tolist(),
                    ...
                    data=df).set(title="PCA-2d projection for mortality")
    plt.show()
    ...
    sns.scatterplot(x="tsne-one", y="tsne-two", hue=df.y.tolist(),
                    palette=sns.color_palette("hls", 2),
                    data=df).set(title="T-SNE projection for mortality") 
    plt.show()
    ...
    sns.scatterplot(x="tsne-one", y="tsne-two", hue=df.y.tolist(),
                    palette=sns.color_palette("hls", 2),
                    data=df).set(title="PCA + T-SNE projection for mortality") 
    plt.show()
    ...
    for row in test_df.itertuples(index=False):
        ...
        for k in range(kmeans.cluster_centers_.shape[0]):
            ...
            # if train_clusters_df[k].shape[0] > 100:
