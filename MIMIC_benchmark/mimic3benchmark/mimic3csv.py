...
def add_age_to_icustays(stays):
    #MODIFIED
    stays['INTIME_TMP'] = stays.INTIME.dt.date
    stays['DOB_TMP'] = stays.DOB.dt.date
    stays['AGE'] = stays.apply(lambda e: (e['INTIME_TMP'] - e['DOB_TMP']).days/365, axis=1)
        #stays['AGE'] = (stays.INTIME - stays.DOB).apply(lambda s: s / np.timedelta64(1, 's')) / 60./60/24/365
    stays.loc[stays.AGE < 0, 'AGE'] = 90
    stays=stays.drop(['DOB_TMP'], axis=1)
    stays=stays.drop(['INTIME_TMP'], axis=1)
    return stays
